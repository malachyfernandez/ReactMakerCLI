...
function basenameWithoutExtension(filePath) {
    return node_path_1.default.basename(filePath).replace(/\.(tsx|ts|jsx|js)$/, "");
}
...
function deriveTreeIgnoreList(baseFileRel, ignoreListProjectRelative) {
    const baseDirRel = normalizeSlashes(node_path_1.default.dirname(baseFileRel));
    return ignoreListProjectRelative.map((item) => {
        const clean = normalizeSlashes(item);
        const rel = normalizeSlashes(node_path_1.default.relative(baseDirRel, clean));
        return rel === "" ? "." : rel;
    });
}
function parseTreeOutput(rawOutput) {
    const rawLines = rawOutput.split(/\r?\n/);
    const lines = rawLines.map((line) => {
        const trimmed = line.trim();
        const fileMatch = trimmed.match(/^<([A-Za-z0-9_.$-]+)>\s*\{\/\*\s*\.(tsx|jsx|ts|js)\s*\*\/\}/);
        if (fileMatch) {
            return {
                raw: line,
                selectable: true,
...
function prefixTreeLine(line) {
    if (line.selectable) {
        return `● ${line.raw}`;
    }
    return `  ${line.raw}`;
}
function computeImportPathFromClonedBaseToMirrored(clonedBaseFileAbs, mirroredTargetAbs) {
    const rel = normalizeSlashes(node_path_1.default.relative(node_path_1.default.dirname(clonedBaseFileAbs), mirroredTargetAbs));
    return ensureDotRelative(stripExtension(rel));
}
async function patchPreviewRootImport(clonedBaseFileAbs, newImportPath) {
    const source = await promises_1.default.readFile(clonedBaseFileAbs, "utf8");
    const previewImportRegex = /import\s+PreviewRoot\s+from\s+["'][^"']+["'];?/;
    ...
    if (previewImportRegex.test(source)) {
        ...
        await promises_1.default.writeFile(clonedBaseFileAbs, next, "utf8");
        return;
    }
    ...
    await promises_1.default.writeFile(clonedBaseFileAbs, lines.join("\n"), "utf8");
}
async function findMirroredMatchesByComponentName(outDir, componentName) {
    const mirroredRoot = node_path_1.default.join(outDir, ".visual-clone", "mirrored");
    const files = await (0, fast_glob_1.default)(["**/*.{tsx,ts,jsx,js}"], {
        cwd: mirroredRoot,
        onlyFiles: true,
        dot: true,
    });
...
function attachStreamLogger(stream, onLine) {
    if (!stream) {
        return;
    }
    let buffer = "";
...
async function ensureToolBuilt(buildCwd, distEntryAbs, log) {
    ...
    if (!(await exists(distEntryAbs))) {
        throw new Error(`Expected built tool at ${distEntryAbs}, but it still does not exist`);
    }
}
async function removeOutDir(outDir, log) {
    if (await exists(outDir)) {
        log(`Removing old output folder: ${outDir}`);
        await promises_1.default.rm(outDir, {
            recursive: true,
            force: true,
        });
    }
}
async function runCloner(opts, log) {
    const args = [
        opts.clonerCli,
        "generate",
        "--project-root",
...
async function installDependencies(opts, log) {
    const manager = opts.packageManager;
    const installArgs = manager === "yarn" ? ["install"] : manager === "pnpm" ? ["install"] : ["install"];
...
async function runTreeTool(opts, log) {
    ...
    log(`react-tree completed. Parsed ${parsed.lines.length} lines, ` +
        `${parsed.selectableCount} selectable file nodes.`);
    return parsed;
}
async function startExpoWeb(opts, log, options = {}) {
    ...
    const availablePort = await (0, detect_port_1.default)(opts.port);
...
class PreviewSwitcherApp {
    constructor(opts) {
        ...
        this.selectedMirroredRelative = null;
        this.selectedComponentName = null;
        this.highlightedComponentName = null;
        ...
        this.treeList = blessed_1.default.list({
            parent: this.screen,
            top: 4,
            left: 0,
            width: "58%",
        ...
        this.logBox = blessed_1.default.log({
            parent: this.screen,
            top: 4,
            left: "58%",
            width: "42%",
        ...
        this.helpBox = blessed_1.default.box({
            parent: this.screen,
            bottom: 0,
            left: 0,
            width: "100%",
    ...
    refreshStatus(extra) {
        ...
        const selectedLabel = currentLine?.selectable && currentLine.componentName
        ...
        const lines = [
            ...
            `Expo web: ${this.currentPort ? `http://localhost:${this.currentPort}` : "not running"}${this.regenerating ? " | rebuilding..." : ""}${extra ? ` | ${extra}` : ""}`,
        ];
    ...
    restoreSelectionAfterTreeUpdate() {
        const candidateNames = [
            this.highlightedComponentName,
            this.selectedComponentName,
        ].filter((name) => Boolean(name));
        for (const name of candidateNames) {
            const index = this.treeLines.findIndex((line) => line.selectable && line.componentName === name);
            if (index >= 0) {
    ...
    moveSelection(delta) {
        ...
        let currentSelectablePosition = selectableIndexes.findIndex((index) => index === current);
    ...
    renderTree(parsed) {
        ...
        this.refreshStatus(`tree loaded (${parsed.selectableCount} selectable file nodes)`);
        this.screen.render();
    }
    async patchToMirroredRelative(mirroredRelativePath) {
        const mirroredAbs = node_path_1.default.join(this.opts.out, ".visual-clone", "mirrored", mirroredRelativePath);
        const importPath = computeImportPathFromClonedBaseToMirrored(this.opts.clonedBaseFileAbs, mirroredAbs);
    ...
    async showMatchChooser(componentName, matches) {
        ...
        return new Promise((resolve) => {
            const outer = blessed_1.default.box({
                parent: this.screen,
                top: "center",
                left: "center",
                width: "80%",
            ...
            const info = blessed_1.default.box({
                parent: outer,
                top: 0,
                left: 1,
                right: 1,
                height: 2,
                content: "Multiple mirrored files match this component name. Choose one and press Enter. Esc cancels.",
            });
    ...
    async openComponentByName(componentName) {
        if (!this.expoReady) {
            const waitMessage = "Expo is still launching. Please wait a moment before opening components.";
    ...
    async restorePreviousSelectionIfPossible() {
        if (this.selectedMirroredRelative) {
            const exact = node_path_1.default.join(this.opts.out, ".visual-clone", "mirrored", this.selectedMirroredRelative);
        ...
        if (this.selectedComponentName) {
            const matches = await findMirroredMatchesByComponentName(this.opts.out, this.selectedComponentName);
            if (matches.length > 0) {
        ...
        const fallbackAbs = node_path_1.default.join(this.opts.out, ".visual-clone", "mirrored", fallback);
        if (await exists(fallbackAbs)) {
            await this.patchToMirroredRelative(fallback);
        }
    }
    selectTreeLineByComponentName(componentName) {
        const index = this.treeLines.findIndex((line) => line.selectable && line.componentName === componentName);
        if (index >= 0) {
            this.selectIndex(index);
        }
    }
    async regenerate(reason, options = {}) {
    ...
    startWatcher() {
        ...
        this.watcher = chokidar_1.default.watch(this.opts.watchDirAbs, {
            ignoreInitial: true,
            awaitWriteFinish: {
    ...
    async run() {
        ...
        await ensureToolBuilt(this.opts.treeBuildCwd, this.opts.treeCli, (line) => this.log(line));
        await ensureToolBuilt(this.opts.clonerBuildCwd, this.opts.clonerCli, (line) => this.log(line));
        if (this.opts.watch) {
            this.startWatcher();
        }
    ...
    startDiagnosticTimer() {
        ...
        this.log(`Diagnostic mode enabled. Auto exit after ${this.opts.diagnosticSeconds}s`);
        this.diagnosticTimer = setTimeout(() => {
            void this.handleDiagnosticTimeout();
        }, this.opts.diagnosticSeconds * 1000);
    }
    async handleDiagnosticTimeout() {
...
function resolveOptions(raw) {
    const toolRoot = node_path_1.default.resolve(__dirname, "..");
    const reactMakerRoot = node_path_1.default.resolve(toolRoot, "..");
    const projectRoot = node_path_1.default.resolve(raw.projectRoot);
    const out = node_path_1.default.resolve(raw.out);
    const baseFile = normalizeSlashes(raw.baseFile);
    const targetComponent = normalizeSlashes(raw.targetComponent);
    const baseFileAbs = node_path_1.default.resolve(projectRoot, baseFile);
    const clonedBaseFileAbs = node_path_1.default.resolve(out, baseFile);
    const baseTopDir = normalizeSlashes(baseFile).split("/")[0] || "app";
    const watchDirAbs = node_path_1.default.resolve(projectRoot, baseTopDir);
    const treeCli = raw.treeCli
        ? node_path_1.default.resolve(raw.treeCli)
        : node_path_1.default.resolve(reactMakerRoot, "react-tree", "React Tree", "dist", "cli.js");
    const clonerCli = raw.clonerCli
        ? node_path_1.default.resolve(raw.clonerCli)
        : node_path_1.default.resolve(reactMakerRoot, "React-compont-visualizer", "react-cloner", "dist", "cli-final.js");
    const treeBuildCwd = node_path_1.default.dirname(node_path_1.default.dirname(treeCli));
    const clonerBuildCwd = node_path_1.default.dirname(node_path_1.default.dirname(clonerCli));
    return {
        projectRoot,
        baseFile,
        targetComponent,
...
async function validateOptions(opts) {
    ...
    const targetAbs = node_path_1.default.resolve(opts.projectRoot, opts.targetComponent);
    if (!(await exists(targetAbs))) {
        throw new Error(`Target component does not exist: ${targetAbs}`);
    }
    if (opts.framework !== "expo-router") {
        throw new Error(`Only --framework expo-router is implemented in this version.`);
    }
    if (Number.isNaN(opts.port) || opts.port <= 0) {
        throw new Error(`Invalid port: ${opts.port}`);
    }
    if (Number.isNaN(opts.watchDebounceMs) || opts.watchDebounceMs < 0) {
        throw new Error(`Invalid watch debounce ms: ${opts.watchDebounceMs}`);
    }
...
async function runConsoleMode(opts) {
    ...
    try {
        ...
        await ensureToolBuilt(opts.treeBuildCwd, opts.treeCli, (line) => pushLog(`[build] ${line}`));
        await ensureToolBuilt(opts.clonerBuildCwd, opts.clonerCli, (line) => pushLog(`[build] ${line}`));
        ...
        pushLog(`💡 To switch previews, edit ${node_path_1.default.join(opts.out, opts.baseFile)} and change the PreviewRoot import.`);
        pushLog(`📂 Mirrored files available in: ${node_path_1.default.join(opts.out, ".visual-clone", "mirrored")}`);
        let watcher = null;
        let diagTimer = null;
        const cleanup = async () => {
            if (diagTimer) {
        ...
        if (opts.watch) {
            ...
            watcher = chokidar_1.default.watch(opts.watchDirAbs, {
                ignoreInitial: true,
                awaitWriteFinish: {
                    stabilityThreshold: 300,
                    pollInterval: 100,
            ...
            watcher.on("all", (eventName, changedPath) => {
                ...
                rebuildTimer = setTimeout(async () => {
                    try {
                        ...
                        pushLog(`❌ Rebuild failed: ${error instanceof Error ? error.message : String(error)}`);
                    }
            ...
            if (opts.diagnosticSeconds && opts.diagnosticSeconds > 0) {
                diagTimer = setTimeout(() => {
                    pushLog(`Diagnostic timer of ${opts.diagnosticSeconds}s elapsed. Exiting...`);
                    void exitWithLogs(0);
...
async function main() {
    ...
    program
        ...
        .description("Wrapper around react-tree and react-cloner to create a live selectable preview switcher.")
        ...
        .option("--package-manager <name>", "Package manager for cloned app install", "npm")
        .option("--watch-debounce-ms <number>", "Debounce delay before rebuild after file changes", "1200")
